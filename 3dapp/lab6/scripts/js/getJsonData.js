$(document).ready(function(){
    ...
    $.getJSON("./model/data.json", function(jsonObj) {
       ...
       //Get the coke main text data
		...
		$('.title_gallery').html('<h2>' + jsonObj.pageTextData[5].galleryTitle + '</h2>');
		$('.description_gallery').html('<p>' + jsonObj.pageTextData[5].galleryDescription + '</p>');
        
        //Get camera contents data
		$('.x3dCameraTitle').html('<h2>' + jsonObj.pageTextData[6].CameraTitle + '</h2>');
		$('.x3dCameraSubtitle').html('<p>' + jsonObj.pageTextData[6].CameraSubtitle + '</p>');

        //Get animation contents data
		$('.x3dAnimationTitle').html('<h2>' + jsonObj.pageTextData[7].AnimationTitle + '</h2>');
		$('.x3dAnimationSubtitle').html('<p>' + jsonObj.pageTextData[7].AnimationSubtitle + '</p>');

        //Get render and lights contents data
		$('.x3dRnlTitle').html('<h2>' + jsonObj.pageTextData[8].RnlTitle + '</h2>');
		$('.x3dRnlSubtitle').html('<p>' + jsonObj.pageTextData[8].RnlSubtitle + '</p>');
    });
});